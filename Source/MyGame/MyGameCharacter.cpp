...
AMyGameCharacter::AMyGameCharacter()
{
	...
	// Configure character movement(Configuracion del movimiento del personaje)
	...
	//"Creando la camara(lleva la camara hacia el jugador si hay una colision)
	CameraBoom = CreateDefaultSubobject<USpringArmComponent>(TEXT("CameraBoom"));
	CameraBoom->SetupAttachment(RootComponent);
	CameraBoom->TargetArmLength = 300.0f; // The camera follows at this distance behind the character	
	CameraBoom->bUsePawnControlRotation = true; // Rotate the arm based on the controller

	// Create a follow camera(Creacion de la camara de seguimiento)
	...
	//Creando linterna o Spotlight del personaje
	Linterna = CreateAbstractDefaultSubobject<USpotLightComponent>(TEXT("Linterna"));
...
//////////////////////////////////////////////////////////////////////////
// Input(Entrada de teclado,controlador)

void AMyGameCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	// Set up gameplay key bindings(Instanciando la entrada de teclado para manejar al jugador)
...
//Funcion linterna del jugador
void AMyGameCharacter::EncenderLinterna(bool encender)
{
	Linterna->SetVisibility(encender);
}
...
/*Manejo de jugador por entrada tactil
bool AMyGameCharacter::EnableTouchscreenMovement(class UInputComponent* PlayerInputComponent)
{
	if (FPlatformMisc::SuportsTouchInput() || GetDefault<UInputSettings>()->bUseMouseForTocuh)
	{
		PlayerInputComponent->BindTouch(EInputEvent::IE_Pressed, this, &AMyGameCharacter::BeginTouch);
		PlayerInputComponent->BindTouch(EInputEvent::IE_Pressed, this, &AMyGameCharacter::EndTouch);
		return true;
	}
	return false;
}
